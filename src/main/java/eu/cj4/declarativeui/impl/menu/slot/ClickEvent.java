...
public record ClickEvent(List<ClickAction> actions, Optional<ClickType> clickType, Optional<ContainerInput> actionType) {
    private static final Codec<ClickType> CLICK_TYPE_CODEC = LazyEnumCodec.fromEnum(ClickType.values());
    private static final Codec<ContainerInput> ACTION_TYPE_CODEC = LazyEnumCodec.fromEnum(ContainerInput.values());
